...
export function BaseNode({ className, status, ...props }: BaseNodeProps) {
  return (
    ...
    >
      ...
      {status === 'success' && (
        <CheckCircleIcon className="size-2 absolute right-0.5 bottom-0.5 text-green-700 stroke-3" />
      )}
      {status === 'loading' && (
        <Loader2Icon className="size-2 absolute right-0.5 bottom-0.5 text-blue-700 stroke-3 animate-spin" />
      )}
    </div>
  );
}
